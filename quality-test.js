...
for (const path of ['/ready','/api/v1/robots','/api/v1/adapters/{provider}/sync','/api/v1/sync-jobs/{jobId}','/api/v1/cenobots/robots/{robotId}/commands','/api/v1/integrations/enterprise','/api/v1/webhooks/service']) assert.ok(openapi.paths[path],`OpenAPI path missing: ${path}`);
...
assert.match(html,/id="moreWorkspacesButton"[^>]+aria-expanded="false"/);
assert.match(html,/class="hero-more-actions"/);
