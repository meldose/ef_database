...
(async () => {
  try {
    ...
    const readyBody=await readyResponse.json(); assert.equal(readyBody.ready,true); assert.ok(readyBody.checks.some((check) => check.name === 'provider-autoxing' && check.detail === 'mock mode')); assert.ok(readyBody.checks.some((check) => check.name === 'provider-cenobots' && check.detail === 'mock mode'));
    ...
    assert.equal(result.body.data.diagnostics.mode,'mock');
    assert.ok(Array.isArray(result.body.data.fleet[0].water.cleanLevels));
    assert.equal(result.body.data.fleet[0].commandAvailability.missionState,'IDLE');
    ...
    process.env.CENOBOTS_LIVE='true'; process.env.CENOBOTS_COMMANDS_ENABLED='true'; process.env.CENOBOTS_ACCESS_KEY='test-access'; process.env.CENOBOTS_SECRET_KEY='test-secret'; state.robots.get(cenobotsRobotId).online=true; state.robots.get(cenobotsRobotId).providerTask={ status:'RUNNING' };
    result=await request(`/api/v1/cenobots/robots/${cenobotsRobotId}/commands`,{ method:'POST',body:JSON.stringify({ action:'go-home',execute:true,confirmation:'CB-DEMO-001' }) }); assert.equal(result.status,409); assert.match(result.body.error.message,/Stop the current mission/);
    process.env.CENOBOTS_LIVE='false'; process.env.CENOBOTS_COMMANDS_ENABLED='false'; delete process.env.CENOBOTS_ACCESS_KEY; delete process.env.CENOBOTS_SECRET_KEY; state.robots.get(cenobotsRobotId).providerTask=null;
