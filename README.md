...
Create coordinated database and object-storage backups with `npm run backup`. Each backup is staged atomically and includes a format manifest plus SHA-256 checksums for the database dump, manifest, and every downloaded object. Restore only after validation with `ALTEGRO_RESTORE_CONFIRM=RESTORE npm run restore -- BACKUP_DIRECTORY`. Prove the backup in a disposable database with `DATABASE_URL=SOURCE ALTEGRO_DRILL_DATABASE_URL=DISPOSABLE npm run verify:backup -- BACKUP_DIRECTORY`; the drill refuses to target the source database and verifies the snapshot and current schema. The scripts require PostgreSQL client tools and the AWS CLI when S3 storage is enabled.
