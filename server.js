...
function executableAvailable(command) {
  ...
  const candidates=value.includes(pathUtil.sep) ? [value] : String(process.env.PATH || '').split(pathUtil.delimiter).filter(Boolean).map((directory) => pathUtil.join(directory,value));
...
function providerRuntimeStatus(provider) {
  const pythonAvailable=executableAvailable(process.env.PYTHON_BIN || 'python3');
  if (provider === 'autoxing') {
    const liveEnabled=autoXingLiveEnabled(); const credentialsConfigured=['APPID','APPSECRET','APPCODE'].every((name) => Boolean(secretEnvValue(name)));
    const checks={ python:pythonAvailable,bridge:fs.existsSync(AUTOXING_BRIDGE),wrapper:fs.existsSync(AUTOXING_LIB),credentials:credentialsConfigured };
    const reasons=[]; if (!checks.python) reasons.push('Python runtime unavailable'); if (!checks.bridge) reasons.push('AutoXing bridge unavailable'); if (!checks.wrapper) reasons.push('AutoXing wrapper mount unavailable'); if (!checks.credentials) reasons.push('AutoXing credentials incomplete');
  ...
  if (provider === 'cenobots') {
    const liveEnabled=cenoBotsLiveEnabled(); const credentialsConfigured=['CENOBOTS_ACCESS_KEY','CENOBOTS_SECRET_KEY'].every((name) => Boolean(secretEnvValue(name)));
    const checks={ python:pythonAvailable,bridge:fs.existsSync(CENOBOTS_BRIDGE),credentials:credentialsConfigured };
...
const ROLE_PERMISSIONS = Object.freeze({
  platform_admin:['robot.read','robot.write','robot.register','report.read','report.export','notification.manage','workforce.read','workforce.manage','work_order.read','work_order.manage','cost.read','cost.manage','audit.read','audit.export','customer_dashboard.read','customer_dashboard.all','integration.read','integration.sync','robot.control','support.create','support.manage'],
  data_admin:['robot.read','robot.write','robot.register','report.read','report.export','notification.manage','workforce.read','workforce.manage','work_order.read','work_order.manage','cost.read','cost.manage','audit.read','audit.export','customer_dashboard.read','customer_dashboard.all','integration.read','integration.sync','support.create','support.manage'],
  support_admin:['robot.read','robot.write','robot.register','report.read','report.export','notification.manage','workforce.read','workforce.manage','work_order.read','work_order.manage','cost.read','cost.manage','audit.read','audit.export','customer_dashboard.read','customer_dashboard.all','integration.read','integration.sync','robot.control','support.create','support.manage'],
  owner:['robot.read','robot.write','robot.register','report.read','report.export','workforce.read','work_order.read','cost.read','audit.read','customer_dashboard.read','support.create'],
  ...
  auditor:['robot.read','report.read','report.export','cost.read','audit.read','audit.export','customer_dashboard.read'],
...
function syncAdapter(provider, actorId = 'system') {
  ...
  return { provider,mode:'mock',adapterVersion:adapter.version,robot:getPassport(robot.id),count:1,changes:{ created:created ? 1 : 0,updated:created ? 0 : 1,unchanged:0,failed:0 },commandCapabilitiesEnabled:false };
}
...
function cenoBotsControlConfiguration() {
  ...
  const runtime=providerRuntimeStatus('cenobots'); const reasons=[];
  if (!runtime.liveEnabled) reasons.push('CenoBots is using mock mode'); if (!commandsEnabled) reasons.push('Live commands are disabled'); if (!credentialsConfigured) reasons.push('CenoBots credentials are incomplete'); if (!runtime.checks.python) reasons.push('Python runtime is unavailable');
  return { liveEnabled:runtime.liveEnabled,mode:runtime.mode,commandsEnabled,credentialsConfigured,ready:runtime.liveEnabled && commandsEnabled && credentialsConfigured && runtime.ready,reasons,safetyConfirmation:'Exact robot serial number required for every live command.' };
}

function cenoBotsCommandAvailability(robot) {
  const task=robot.providerTask && typeof robot.providerTask === 'object' ? robot.providerTask : {};
  const stateValue=String(task.status || task.state || task.taskStatus || task.missionStatus || '').toUpperCase();
  const completeStates=new Set(['','COMPLETED','COMPLETE','FINISHED','SUCCEEDED','SUCCESS','FAILED','CANCELLED','CANCELED','STOPPED','IDLE']);
  const active=Object.keys(task).length > 0 && !completeStates.has(stateValue); const paused=active && stateValue.includes('PAUS');
  const unsafe=robot.emergencyStop === true || robot.manualMode === true; const offline=robot.online !== true;
  return { missionActive:active,missionPaused:paused,missionState:stateValue || (active ? 'ACTIVE' : 'IDLE'),actions:{
    clean:{ allowed:!offline && !unsafe && !active && Boolean(robot.providerMapId && robot.providerMapVersion),reason:offline ? 'Robot is offline' : unsafe ? 'Clear emergency stop and manual mode first' : active ? 'Stop the current mission first' : !robot.providerMapId || !robot.providerMapVersion ? 'Current map information is unavailable' : null },
    'go-home':{ allowed:!offline && !unsafe && !active && robot.docked !== true,reason:offline ? 'Robot is offline' : unsafe ? 'Clear emergency stop and manual mode first' : active ? 'Stop the current mission before returning home' : robot.docked === true ? 'Robot is already docked' : null },
    pause:{ allowed:!offline && active && !paused,reason:offline ? 'Robot is offline' : !active ? 'No active mission to pause' : paused ? 'Mission is already paused' : null },
    continue:{ allowed:!offline && active && paused,reason:offline ? 'Robot is offline' : !active ? 'No active mission to continue' : !paused ? 'Mission is not paused' : null },
    stop:{ allowed:!offline && active,reason:offline ? 'Robot is offline' : !active ? 'No active mission to stop' : null },
    schedule:{ allowed:true,reason:null },
  } };
}
...
function cenoBotsOperations(actor) {
  ...
  for (const item of fleet) { const robot=state.robots.get(item.id); const docking=robot?.statusDetails?.dockingStationDetail || {}; item.water={ cleanLevels:clone(robot?.statusDetails?.waterBox || []),dirtyLevels:clone(robot?.statusDetails?.dirtyBox || []),cleanStatus:docking.cleanWaterStatus || null,dirtyStatus:docking.dirtyWaterStatus || null }; item.commandAvailability=cenoBotsCommandAvailability(robot); }
  const adapter=state.adapters.get('cenobots') || {}; const policy=managedSecretStatus().providers.find((item) => item.provider === 'cenobots') || null; const runtime=providerRuntimeStatus('cenobots');
  return { summary:{ total:fleet.length,online:fleet.filter((item) => item.online === true).length,offline:fleet.filter((item) => item.online === false).length,charging:fleet.filter((item) => item.charging === true).length,docked:fleet.filter((item) => item.docked === true).length,waterServicing:fleet.filter((item) => item.water.cleanStatus || item.water.dirtyStatus).length,maintenanceDue:fleet.reduce((sum,item) => sum+item.maintenanceDueCount,0),errors:fleet.reduce((sum,item) => sum+item.errorCount,0) },fleet,alerts:alerts.sort((a,b) => Date.parse(b.occurredAt || 0)-Date.parse(a.occurredAt || 0)),control:{ ...cenoBotsControlConfiguration(),canControl:canControlCenoBots(actor) },diagnostics:{ mode:runtime.mode,runtime,liveEnabled:cenoBotsLiveEnabled(),status:adapter.status,lastSyncAt:adapter.lastSyncAt,lastSyncStatus:adapter.lastSyncStatus,lastError:adapter.lastError ? 'Synchronization failed. Retry or inspect the protected server log.' : null,lastSyncDurationMs:adapter.lastSyncDurationMs,lastSyncCount:adapter.lastSyncCount,lastSyncWarnings:adapter.lastSyncWarnings || 0,minimumRequestIntervalSeconds:Number(process.env.CENOBOTS_MIN_REQUEST_INTERVAL_SECONDS || 1.05),secretPolicy:policy,webhook:cenoBotsWebhookConfiguration(),syncHistory:clone(adapter.syncHistory || []) },generatedAt:timestamp() };
}
...
async function syncAutoXingLive(actor) {
  ...
  const synced = []; let createdCount=0; let updatedCount=0;
...
function costCurrency() { return String(process.env.ALTEGRO_COST_CURRENCY || 'EUR').trim().toUpperCase(); }
function costEntryInput(input,actor) {
  const robot=state.robots.get(String(input.robotId || '')); if (!robot || !visibleToActor(actor,robot)) throw httpError(404,'Robot not found');
  const category=String(input.category || ''); if (!['labor','parts','travel','subscription','other'].includes(category)) throw httpError(400,'category must be labor, parts, travel, subscription, or other');
  const quantity=Number(input.quantity); const unitCostCents=Number(input.unitCostCents); const taxRateBasisPoints=Number(input.taxRateBasisPoints ?? 1900);
  if (!Number.isFinite(quantity) || quantity <= 0 || quantity > 100000 || Math.round(quantity*1000) !== quantity*1000) throw httpError(400,'quantity must be positive with at most three decimal places');
  if (!Number.isSafeInteger(unitCostCents) || unitCostCents < 0 || unitCostCents > 100000000) throw httpError(400,'unitCostCents must be a non-negative integer');
  if (!Number.isInteger(taxRateBasisPoints) || taxRateBasisPoints < 0 || taxRateBasisPoints > 10000) throw httpError(400,'taxRateBasisPoints must be an integer from 0 to 10000');
  const currency=String(input.currency || costCurrency()).toUpperCase(); if (currency !== costCurrency()) throw httpError(400,`currency must be ${costCurrency()}`);
  const serviceCaseId=input.serviceCaseId ? String(input.serviceCaseId) : null; const serviceCase=serviceCaseId ? state.serviceCases.get(serviceCaseId) : null; if (serviceCaseId && (!serviceCase || serviceCase.robotId !== robot.id)) throw httpError(400,'serviceCaseId must reference this robot');
  const workOrderId=input.workOrderId ? String(input.workOrderId) : null; const workOrder=workOrderId ? state.workOrders.get(workOrderId) : null; if (workOrderId && (!workOrder || workOrder.robotId !== robot.id)) throw httpError(400,'workOrderId must reference this robot');
  const occurredAt=new Date(input.occurredAt || Date.now()); if (Number.isNaN(occurredAt.getTime()) || occurredAt.getTime() > Date.now()+86400000) throw httpError(400,'occurredAt must be a valid date that is not in the future');
  const description=String(input.description || '').trim(); if (!description || description.length > 300) throw httpError(400,'description must contain 1 to 300 characters');
  const netCents=Math.round(quantity*unitCostCents); const taxCents=Math.round(netCents*taxRateBasisPoints/10000);
  return { robotId:robot.id,tenantId:robot.tenantId,category,description,quantity,unitCostCents,taxRateBasisPoints,netCents,taxCents,grossCents:netCents+taxCents,currency,serviceCaseId,workOrderId,vendor:String(input.vendor || '').trim().slice(0,160) || null,reference:String(input.reference || '').trim().slice(0,120) || null,occurredAt:occurredAt.toISOString() };
}
function costEntryView(entry) { const robot=state.robots.get(entry.robotId); return { ...clone(entry),robotSerialNumber:robot?.serialNumber || null }; }
function costReport(actor,days=30) {
  days=Number(days || 30); if (!Number.isInteger(days) || days < 1 || days > 365) throw httpError(400,'days must be an integer between 1 and 365'); const since=Date.now()-days*86400000; const robotIds=visibleRobotIds(actor); const entries=[...state.costEntries.values()].filter((item) => robotIds.has(item.robotId) && Date.parse(item.occurredAt) >= since).sort((a,b) => Date.parse(b.occurredAt)-Date.parse(a.occurredAt)); const active=entries.filter((item) => item.status === 'active'); const totals=active.reduce((sum,item) => ({ netCents:sum.netCents+item.netCents,taxCents:sum.taxCents+item.taxCents,grossCents:sum.grossCents+item.grossCents }),{ netCents:0,taxCents:0,grossCents:0 });
  const byCategory=['labor','parts','travel','subscription','other'].map((category) => ({ category,grossCents:active.filter((item) => item.category === category).reduce((sum,item) => sum+item.grossCents,0),count:active.filter((item) => item.category === category).length }));
  const byRobot=[...robotIds].map((robotId) => { const items=active.filter((item) => item.robotId === robotId); return { robotId,serialNumber:state.robots.get(robotId)?.serialNumber || robotId,grossCents:items.reduce((sum,item) => sum+item.grossCents,0),count:items.length }; }).filter((item) => item.count).sort((a,b) => b.grossCents-a.grossCents);
  return { period:{ days,from:new Date(since).toISOString(),to:timestamp() },currency:costCurrency(),totals,count:active.length,voidedCount:entries.length-active.length,byCategory,byRobot,entries:entries.map(costEntryView),permissions:{ manage:hasPermission(actor,'cost.manage') },generatedAt:timestamp() };
}
...
function sendPrometheusMetrics(res) {
  ...
  for (const adapter of snapshot.adapters) {
    lines.push(`altegro_adapter_last_sync_success{provider="${adapter.provider}",mode="${adapter.mode}"} ${adapter.lastSyncStatus === 'success' ? 1 : 0}`);
    lines.push(`altegro_adapter_runtime_ready{provider="${adapter.provider}",mode="${adapter.mode}"} ${adapter.runtimeReady ? 1 : 0}`);
    lines.push(`altegro_adapter_last_sync_timestamp_seconds{provider="${adapter.provider}"} ${adapter.lastSyncAt ? Math.floor(Date.parse(adapter.lastSyncAt)/1000) : 0}`);
    lines.push(`altegro_adapter_last_sync_warnings{provider="${adapter.provider}"} ${adapter.lastSyncWarnings || 0}`);
  }
...
function serveFrontend(pathname, res) {
  const dashboardRoute=/^\/dashboard(?:\/(overview|tracking|robots|operations|autoxing|cenobots|workforce|reports|support|admin))?\/?$/.test(pathname); const requested = pathname === '/' || dashboardRoute ? 'index.html' : pathname === '/docs' || pathname === '/docs/' ? 'docs.html' : pathname.slice(1);
  const allowed = new Set(['index.html', 'app.js', 'styles.css', 'docs.html', 'docs.js', 'openapi.json']);
  if (!allowed.has(requested)) return false;
  const filePath = pathUtil.join(publicDir, requested);
  try {
    const body = fs.readFileSync(filePath);
    const contentTypes = { 'index.html':'text/html; charset=utf-8','docs.html':'text/html; charset=utf-8','app.js':'text/javascript; charset=utf-8','docs.js':'text/javascript; charset=utf-8','styles.css':'text/css; charset=utf-8','openapi.json':'application/json; charset=utf-8' };
...
async function handle(req, res) {
  ...
  if (req.method === 'GET' && path === '/api/v1/costs') { if (!hasPermission(actor,'cost.read')) throw httpError(403,'Cost read permission required'); return send(res,200,{ data:costReport(actor,url.searchParams.get('days')) }); }
  if (req.method === 'GET' && path === '/api/v1/costs.csv') { if (!hasPermission(actor,'cost.read') || !canExport(actor)) throw httpError(403,'Cost export permission required'); const report=costReport(actor,url.searchParams.get('days')); const rows=[['occurred_at','robot','category','description','quantity','unit_cost_cents','net_cents','tax_cents','gross_cents','currency','status'],...report.entries.map((item) => [item.occurredAt,item.robotSerialNumber,item.category,item.description,item.quantity,item.unitCostCents,item.netCents,item.taxCents,item.grossCents,item.currency,item.status])]; recordAudit(actor,'cost.export','tenant',actor.tenantId,'success',{ days:report.period.days,count:report.count }); return sendDownload(res,'text/csv; charset=utf-8',`altegro-costs-${report.period.days}d.csv`,`${rows.map((row) => row.map(csvCell).join(',')).join('\n')}\n`); }
  if (req.method === 'POST' && path === '/api/v1/costs') {
    if (!hasPermission(actor,'cost.manage')) throw httpError(403,'Cost management permission required'); const input=costEntryInput(body,actor); const createdAt=timestamp(); const entry={ id:ids(),...input,status:'active',createdBy:actor.id,createdByName:actor.name,createdAt,updatedAt:createdAt,voidedAt:null,voidedBy:null,voidReason:null }; state.costEntries.set(entry.id,entry); appendPassportEntry(entry.robotId,{ type:'cost_recorded',source:'altegro-costs',occurredAt:entry.occurredAt,data:{ costEntryId:entry.id,category:entry.category,description:entry.description,grossCents:entry.grossCents,currency:entry.currency,serviceCaseId:entry.serviceCaseId,workOrderId:entry.workOrderId } },actor); appendOutbox('cost.recorded','cost_entry',entry.id,{ robotId:entry.robotId,grossCents:entry.grossCents,currency:entry.currency }); recordAudit(actor,'cost.create','cost_entry',entry.id,'success',{ robotId:entry.robotId,grossCents:entry.grossCents,currency:entry.currency }); return send(res,201,{ data:costEntryView(entry) });
  }
  const costUpdate=route(req.method,path,/^\/api\/v1\/costs\/([^/]+)$/);
  if (costUpdate && req.method === 'PATCH') {
    if (!hasPermission(actor,'cost.manage')) throw httpError(403,'Cost management permission required'); const entry=state.costEntries.get(decodeURIComponent(costUpdate.id)); if (!entry || !visibleRobotIds(actor).has(entry.robotId)) throw httpError(404,'Cost entry not found'); if (entry.status !== 'active') throw httpError(409,'Cost entry is already voided'); const reason=String(body.reason || '').trim(); if (reason.length < 3 || reason.length > 300) throw httpError(400,'A void reason of 3 to 300 characters is required'); Object.assign(entry,{ status:'voided',voidedAt:timestamp(),voidedBy:actor.id,voidReason:reason,updatedAt:timestamp() }); appendPassportEntry(entry.robotId,{ type:'cost_voided',source:'altegro-costs',data:{ costEntryId:entry.id,reason } },actor); appendOutbox('cost.voided','cost_entry',entry.id,{ robotId:entry.robotId,reason }); recordAudit(actor,'cost.void','cost_entry',entry.id,'success',{ reason }); return send(res,200,{ data:costEntryView(entry) });
  }
  ...
  if (cenoBotsCommand && req.method === 'POST') {
    ...
    if (execute) {
      ...
      const availability=cenoBotsCommandAvailability(robot).actions[action]; if (!availability?.allowed) { recordAudit(actor,'cenobots.command','robot',robot.id,'rejected',{ action,reason:'state-conflict' }); throw httpError(409,availability?.reason || 'The command conflicts with the current robot state'); }
    }
...
assertLiveProviderRuntimeConfiguration();
...
async function processSyncJobs() {
  ...
  try {
    ...
    try {
      ...
      const finalStatus=(result.resourceErrors || []).length ? 'partial' : 'completed'; const compact={ status:finalStatus,provider:job.provider,mode:result.mode || providerRuntimeStatus(job.provider).mode,count:result.count ?? 1,changes:result.changes || null,resources:result.resources || null,resourceErrors:result.resourceErrors || [],completedAt:timestamp() };
      await databaseStore.completeSyncJob(job.id,compact,finalStatus); recordAudit(actor,'adapter.sync.completed','adapter',job.provider,finalStatus === 'partial' ? 'warning' : 'success',{ jobId:job.id,count:compact.count }); await persistState();
