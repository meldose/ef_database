...
(async () => {
  ...
  try {
    ...
    const costCount=state.costEntries.size; result=await request('/api/v1/costs','POST',{ robotId:ax.id,category:'labor',description:'Invalid fractional cents',quantity:1,unitCostCents:1.5,occurredAt:new Date().toISOString() }); assert.equal(result.status,400); assert.equal(state.costEntries.size,costCount);
    result=await request('/api/v1/costs','POST',{ robotId:ax.id,category:'labor',description:'Annual inspection labour',quantity:1.5,unitCostCents:10000,taxRateBasisPoints:1900,occurredAt:new Date().toISOString(),vendor:'Demo Robot Care',reference:'WO-100' }); assert.equal(result.status,201); const costId=result.body.data.id; assert.equal(result.body.data.netCents,15000); assert.equal(result.body.data.taxCents,2850); assert.equal(result.body.data.grossCents,17850);
    result=await request('/api/v1/costs?days=30','GET',undefined,'demo-owner'); assert.equal(result.status,200); assert.equal(result.body.data.totals.grossCents,17850); assert.equal(result.body.data.entries[0].robotSerialNumber,ax.serialNumber);
    result=await request('/api/v1/reports/operations?days=30'); assert.equal(result.status,200); assert.equal(result.body.data.service.costTrackingAvailable,true); assert.equal(result.body.data.service.costs.grossCents,17850);
    result=await request('/api/v1/costs?days=30','GET',undefined,'demo-robot-ax-001'); assert.equal(result.status,403);
    result=await request('/api/v1/costs','POST',{ robotId:ax.id,category:'parts',description:'Owner write',quantity:1,unitCostCents:100 },'demo-owner'); assert.equal(result.status,403);
    result=await request(`/api/v1/costs/${costId}`,'PATCH',{ reason:'no' }); assert.equal(result.status,400); assert.equal(state.costEntries.get(costId).status,'active');
    result=await request(`/api/v1/costs/${costId}`,'PATCH',{ reason:'Duplicate supplier invoice' },'demo-owner'); assert.equal(result.status,403);
    result=await request(`/api/v1/costs/${costId}`,'PATCH',{ reason:'Duplicate supplier invoice' }); assert.equal(result.status,200); assert.equal(result.body.data.status,'voided');
    result=await request('/api/v1/costs?days=30'); assert.equal(result.body.data.totals.grossCents,0); assert.equal(result.body.data.voidedCount,1);
