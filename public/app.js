...
const state = { token:null,user:null,robots:[],robotOptions:[],serviceCases:[],compatibility:[],notifications:[],notificationActiveCount:0,selectedNotificationId:null,reports:null,audit:[],autoXingResources:null,autoXingTasks:[],autoXingOperations:null,cenoBotsOperations:null,maintenanceSchedules:[],escalationRules:null,monitoring:null,emailNotifications:null,selectedAlertId:null,onboardingStep:1,autoXingFleet:{ query:'',status:'all',battery:'all',alerts:'all',page:1,pageSize:12 },cenoBotsFleet:{ query:'',status:'all',attention:'all' },robotAccounts:[],workforce:null,summary:null,selectedRobotId:null,registry:{ page:1,pageSize:10,pageCount:1,count:0 },sync:{ provider:null,startedAt:0,timer:null },autoRefresh:{ timer:null,intervalMs:30000,lastAt:null } };
...
const dashboardViews = new Set(['overview', 'robots', 'operations', 'autoxing', 'cenobots', 'workforce', 'reports', 'admin']);
const roleViews = {
  robot_user:['overview','robots','operations','reports'],
  auditor:['overview','robots','operations','reports','admin'],
  owner:['overview','robots','operations','reports'],
  technician:['overview','robots','operations','workforce','reports'],
...
function renderRoleDashboard() {
  const profiles={
    ...
    owner:{ title:'Customer fleet overview',copy:'Track fleet availability, service progress and lifecycle compliance.',message:'Review offline robots, open service cases and operational performance.',actions:[['robots','My robots'],['operations','Service status'],['reports','Performance report']] },
    ...
    robot_user:{ title:'My robot dashboard',copy:'A scoped operational view for the robots assigned to this account.',message:'Review current state, technical history and your Robot Passport.',actions:[['robots','Open my robot'],['operations','Technical events'],['reports','Robot report']] },
  };
  const profile=profiles[state.user.role] || profiles.owner; $('#roleDashboardTitle').textContent=profile.title; $('#roleDashboardCopy').textContent=profile.copy; $('#roleDashboardEyebrow').textContent=`${state.user.role.replaceAll('_',' ')} workspace`; $('#roleDashboardMessage').textContent=profile.message; $('#roleDashboardActions').innerHTML=profile.actions.map(([view,label]) => `<button type="button" class="button secondary" data-role-target="${view}">${escapeHtml(label)}</button>`).join(''); $('#roleDashboardActions').querySelectorAll('[data-role-target]').forEach((button) => button.addEventListener('click',() => setDashboardView(button.dataset.roleTarget)));
}
...
async function loadNotifications() {
  ...
  state.notificationActiveCount=payload.activeCount ?? payload.count; $('#notificationCount').textContent = state.notificationActiveCount > 99 ? '99+' : String(state.notificationActiveCount);
  renderNotifications();
}
...
async function loadReports() {
  ...
  const daily=data.daily || []; const max=Math.max(1,...daily.map((item) => item.events)); $('#reportTrend').innerHTML=daily.map((item) => `<div class="report-day" title="${escapeHtml(item.date)}: ${item.events} events, ${item.errors} errors"><span style="height:${Math.max(3,item.events/max*100)}%" class="${item.errors ? 'has-error' : ''}"></span><small>${escapeHtml(item.date.slice(5))}</small></div>`).join('');
}
