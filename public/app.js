...
function expireSession(message = 'Your session expired. Please sign in again.') {
  sessionStorage.removeItem('altegroSession'); clearInterval(state.autoRefresh.timer); clearInterval(state.trackingRefresh.timer); clearTimeout(state.notificationReconnectTimer); state.notificationSocket?.close(); state.notificationSocket=null; state.autoRefresh.timer = null; state.trackingRefresh.timer=null; state.token = null; state.user = null; state.robots = []; state.selectedRobotId = null; showLogin(); $('#loginError').textContent = message;
}

function showApp() {
  $('#loginScreen').classList.add('hidden'); $('#appShell').classList.remove('hidden');
  $('#loggedInUser').textContent = `${state.user.name} · ${state.user.role.replaceAll('_', ' ')}`;
  const robotUser = state.user.role === 'robot_user'; const auditor = state.user.role === 'auditor'; const can=(permission) => state.user.permissions?.includes(permission);
  ['syncAll', 'syncAutoXing', 'syncCenoBots', 'syncCenoBotsWorkspace'].forEach((id) => { const element = $(`#${id}`); if (element) element.classList.toggle('hidden',!can('integration.sync')); });
  $('#newRobotButton').classList.toggle('hidden',!can('robot.register'));
  $('#exportTenant').classList.toggle('hidden',!can('report.export'));
  $('#exportRobotsCsv').classList.toggle('hidden',!can('report.export'));
  $('#serviceTechniciansButton').classList.toggle('hidden',!can('workforce.read'));
  $('#exportAudit').classList.toggle('hidden',!can('audit.export'));
  ...
  $('#newTechnicianButton').classList.toggle('hidden',!can('workforce.manage'));
  ...
  renderRoleDashboard(); const routedView=location.pathname.match(/^\/dashboard\/([^/]+)\/?$/)?.[1]; setDashboardView(routedView || sessionStorage.getItem('altegroDashboardView') || 'overview',{ replace:true });
}

function setDashboardView(requestedView,{ replace=false,skipHistory=false }={}) {
  ...
  if (!skipHistory) { const path=`/dashboard/${view}`; if (location.pathname !== path) history[replace ? 'replaceState' : 'pushState']({ view },'',path); }
...
async function completeLogin(payload) {
  ...
  showApp(); await refreshAll(); startAutoRefresh(); startTrackingRefresh(); startNotificationSocket();
}
...
async function restoreSession() {
  try {
    ...
    const payload = await response.json();
    state.token = saved.token; state.user = payload.user; showApp(); await refreshAll(); startAutoRefresh(); startTrackingRefresh(); startNotificationSocket();
    return true;
  } catch {
...
function startNotificationSocket() { clearTimeout(state.notificationReconnectTimer); state.notificationSocket?.close(); if (!state.token) return; const protocol=location.protocol === 'https:' ? 'wss:' : 'ws:'; const socket=new WebSocket(`${protocol}//${location.host}/api/v1/notifications/stream`); state.notificationSocket=socket; socket.addEventListener('message',async (event) => { try { const message=JSON.parse(event.data); if (message.type !== 'notifications.changed') return; const previous=state.notificationUnreadCount; updateNotificationBadge(message.unreadCount); if (message.unreadCount > previous && message.latest?.[0]) toast(`${message.latest[0].severity.toUpperCase()}: ${message.latest[0].title}`,message.latest[0].severity === 'critical'); await loadNotifications(); } catch {} }); socket.addEventListener('close',() => { if (state.notificationSocket === socket) state.notificationSocket=null; if (state.token) state.notificationReconnectTimer=setTimeout(startNotificationSocket,3000); }); socket.addEventListener('error',() => socket.close()); }

function renderNotifications() {
  const query=$('#notificationSearch')?.value.trim().toLowerCase() || ''; const severity=$('#notificationSeverity')?.value || ''; const status=$('#notificationStatus')?.value || ''; const writable=state.user.permissions?.includes('notification.manage');
...
async function loadReports() {
  ...
  $('#advancedAnalytics').innerHTML=`<article class="analytics-card"><h3>Fleet health</h3><p><span>Health score</span><strong>${metric(data.fleet.healthScore,'%')}</strong></p><p><span>Needs attention</span><strong>${data.fleet.attentionRobots}</strong></p><p><span>Low battery</span><strong>${data.fleet.lowBattery}</strong></p></article><article class="analytics-card"><h3>Service performance</h3><p><span>Closure rate</span><strong>${metric(data.service.caseClosureRate,'%')}</strong></p><p><span>Average resolution</span><strong>${metric(data.service.averageResolutionHours,' h')}</strong></p><p><span>Incidents / robot</span><strong>${metric(data.events.incidentsPerRobot)}</strong></p></article><article class="analytics-card"><h3>Provider & workforce</h3>${data.fleet.providers.map((item) => `<p><span>${escapeHtml(item.provider)}</span><strong>${item.count}</strong></p>`).join('')}<p><span>Technician availability</span><strong>${metric(data.workforce.availabilityPercent,'%')}</strong></p><p><span>Predicted high risk</span><strong>${data.maintenance?.predictedHighRisk || 0}</strong></p></article>`;
...
async function loadMaintenancePredictions() { const payload=await api('/api/v1/maintenance/predictions'); state.predictions=payload; const items=payload.data.slice(0,8); $('#maintenancePredictions').innerHTML=items.length ? items.map((item) => `<article class="prediction-card risk-${escapeHtml(item.risk)}"><div><strong>${escapeHtml(item.serialNumber)}</strong><span>${escapeHtml(item.risk)} risk · ${item.score}%</span></div><p>${escapeHtml(item.factors.join(' · ') || 'No current warning factors.')}</p><small>Recommended window: ${item.predictedWindowDays} days</small></article>`).join('') : '<div class="empty">No robots are available for prediction.</div>'; }

async function loadCustomerDashboard() { if (!state.user.permissions?.includes('customer_dashboard.read')) return; const selected=$('#customerDashboardSelector').value; const payload=await api(`/api/v1/customer-dashboard${selected ? `?organizationId=${encodeURIComponent(selected)}` : ''}`); state.customerDashboard=payload.data; const data=payload.data; const selector=$('#customerDashboardSelector'); selector.innerHTML=(data.customers.length ? data.customers : [data.customer]).map((item) => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join(''); selector.value=data.customer.id; $('#customerDashboardSelectorLabel').classList.toggle('hidden',data.customers.length <= 1); $('#customerDashboardTitle').textContent=`${data.customer.name} dashboard`; $('#customerDashboardContent').innerHTML=`<div class="customer-metrics"><div class="resource-metric"><strong>${data.fleet.total}</strong><span>Robots</span></div><div class="resource-metric"><strong>${data.fleet.online}</strong><span>Online</span></div><div class="resource-metric"><strong>${data.service.open}</strong><span>Open cases</span></div><div class="resource-metric"><strong>${data.maintenance.highRisk}</strong><span>High risk</span></div></div><div class="customer-sites">${data.sites.map((site) => `<article><strong>${escapeHtml(site.name)}</strong><span>${site.online}/${site.total} online · ${site.attention} need attention</span></article>`).join('') || '<div class="empty">No customer sites have robots.</div>'}</div>`; }

function currentIsoWeek() { const date=new Date(); const day=(date.getUTCDay()+6)%7; date.setUTCDate(date.getUTCDate()-day+3); const firstThursday=new Date(Date.UTC(date.getUTCFullYear(),0,4)); const week=1+Math.round((date-firstThursday)/604800000); return `${date.getUTCFullYear()}-W${String(week).padStart(2,'0')}`; }
function weekRange(value) { const match=String(value).match(/^(\d{4})-W(\d{2})$/); if (!match) return null; const year=Number(match[1]),week=Number(match[2]); const fourth=new Date(Date.UTC(year,0,4)); const monday=new Date(fourth); monday.setUTCDate(fourth.getUTCDate()-((fourth.getUTCDay()+6)%7)+(week-1)*7); return [monday.getTime(),monday.getTime()+7*86400000]; }
function renderWorkOrders() { const range=weekRange($('#workOrderWeek').value); const orders=state.workOrders.filter((item) => !range || Date.parse(item.startsAt) >= range[0] && Date.parse(item.startsAt) < range[1]); $('#newWorkOrder').classList.toggle('hidden',!state.workOrderPermissions?.manage); $('#workOrderCalendar').innerHTML=orders.length ? orders.map((item) => { const canUpdate=state.workOrderPermissions?.manage || state.workOrderPermissions?.updateAssigned; const next={ scheduled:'in_progress',in_progress:'completed',blocked:'in_progress' }[item.status]; return `<article class="work-order-card priority-${escapeHtml(item.priority)}"><time>${formatDate(item.startsAt)}–${new Date(item.endsAt).toLocaleTimeString([], { hour:'2-digit',minute:'2-digit' })}</time><div><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.robotSerialNumber)} · ${escapeHtml(item.technicianName || 'Unassigned')}</small></div><span class="status status-${item.status === 'completed' ? 'active' : 'draft'}">${escapeHtml(item.status.replaceAll('_',' '))}</span>${canUpdate && next ? `<button class="text-button" type="button" data-work-order-status="${escapeHtml(next)}" data-work-order-id="${escapeHtml(item.id)}">Mark ${escapeHtml(next.replaceAll('_',' '))}</button>` : ''}</article>`; }).join('') : '<div class="empty">No work orders in this week.</div>'; }
async function loadWorkOrders() { if (!state.user.permissions?.includes('work_order.read')) return; const payload=await api('/api/v1/work-orders'); state.workOrders=payload.data; state.workOrderPermissions=payload.permissions; if (!$('#workOrderWeek').value) $('#workOrderWeek').value=currentIsoWeek(); renderWorkOrders(); }
function populateWorkOrderTechnicians() { const robotId=$('#workOrderRobot').value; const rows=(state.workforce?.rows || []).filter((row) => row.robot.id === robotId && row.eligibility.eligible && !['off_duty','on_leave'].includes(row.technician.availability?.status)); $('#workOrderTechnician').innerHTML=rows.map((row) => `<option value="${escapeHtml(row.technician.id)}">${escapeHtml(row.technician.name)} · ${escapeHtml(row.technician.availability?.status || 'available')}</option>`).join(''); }
function openWorkOrderDialog() { $('#workOrderForm').reset(); $('#workOrderRobot').innerHTML=state.robotOptions.map((robot) => `<option value="${escapeHtml(robot.id)}">${escapeHtml(robot.serialNumber)}</option>`).join(''); const starts=new Date(Date.now()+86400000); starts.setMinutes(0,0,0); $('#workOrderStarts').value=toDateTimeLocal(starts); $('#workOrderEnds').value=toDateTimeLocal(new Date(starts.getTime()+2*3600000)); populateWorkOrderTechnicians(); $('#workOrderDialog').showModal(); }

async function loadPermissions() { const payload=await api('/api/v1/permissions'); state.permissionsData=payload.data; $('#permissionMatrix').innerHTML=payload.data.roles.map((item) => `<article><strong>${escapeHtml(item.role.replaceAll('_',' '))}</strong><div>${item.permissions.map((permission) => `<span class="${item.role === payload.data.role ? 'current' : ''}">${escapeHtml(permission)}</span>`).join('')}</div></article>`).join(''); }
...
async function loadAudit() {
  if (!state.user.permissions?.includes('audit.read')) return; const params=new URLSearchParams({ limit:'100' }); const values={ q:$('#auditSearch').value.trim(),action:$('#auditAction').value,result:$('#auditResult').value,from:$('#auditFrom').value ? new Date(`${$('#auditFrom').value}T00:00:00`).toISOString() : '',to:$('#auditTo').value ? new Date(`${$('#auditTo').value}T23:59:59.999`).toISOString() : '' }; Object.entries(values).forEach(([key,value]) => { if (value) params.set(key,value); }); const payload=await api(`/api/v1/audit?${params}`); state.audit=payload.data; const actionValue=$('#auditAction').value,resultValue=$('#auditResult').value; $('#auditAction').innerHTML='<option value="">All actions</option>'+payload.facets.actions.map((item) => `<option value="${escapeHtml(item)}">${escapeHtml(item.replaceAll('.',' '))}</option>`).join(''); $('#auditResult').innerHTML='<option value="">All results</option>'+payload.facets.results.map((item) => `<option value="${escapeHtml(item)}">${escapeHtml(item)}</option>`).join(''); $('#auditAction').value=actionValue; $('#auditResult').value=resultValue; $('#auditList').innerHTML=payload.data.length ? payload.data.map((item) => `<div class="record-row"><div><strong>${escapeHtml(item.action.replaceAll('.', ' '))}</strong><small>${escapeHtml(item.actorName)} · ${escapeHtml(item.objectType)} · ${escapeHtml(item.result)}</small></div><time class="event-time" datetime="${escapeHtml(item.occurredAt)}">${formatDate(item.occurredAt)}</time></div>`).join('') : '<div class="empty">No audit records match these filters.</div>';
}
...
function bindUi() {
  window.addEventListener('popstate',() => { const view=location.pathname.match(/^\/dashboard\/([^/]+)\/?$/)?.[1] || 'overview'; setDashboardView(view,{ skipHistory:true }); });
  ...
  let auditSearchTimer; $('#auditSearch').addEventListener('input',() => { clearTimeout(auditSearchTimer); auditSearchTimer=setTimeout(() => loadAudit().catch((error) => toast(error.message,true)),200); }); ['auditAction','auditResult','auditFrom','auditTo'].forEach((id) => $(`#${id}`).addEventListener('change',() => loadAudit().catch((error) => toast(error.message,true)))); $('#clearAuditFilters').addEventListener('click',() => { ['auditSearch','auditAction','auditResult','auditFrom','auditTo'].forEach((id) => { $(`#${id}`).value=''; }); loadAudit(); }); $('#exportAudit').addEventListener('click',() => { const params=new URLSearchParams(); const values={ q:$('#auditSearch').value.trim(),action:$('#auditAction').value,result:$('#auditResult').value,from:$('#auditFrom').value ? new Date(`${$('#auditFrom').value}T00:00:00`).toISOString() : '',to:$('#auditTo').value ? new Date(`${$('#auditTo').value}T23:59:59.999`).toISOString() : '' }; Object.entries(values).forEach(([key,value]) => { if (value) params.set(key,value); }); download(`/api/v1/audit.csv?${params}`,'altegro-audit.csv').catch((error) => toast(error.message,true)); });
  $('#customerDashboardSelector').addEventListener('change',() => loadCustomerDashboard().catch((error) => toast(error.message,true)));
  ...
  $('#refreshPredictions').addEventListener('click',() => loadMaintenancePredictions().catch((error) => toast(error.message,true)));
  ...
  $('#refreshWorkOrders').addEventListener('click',() => loadWorkOrders().catch((error) => toast(error.message,true))); $('#workOrderWeek').addEventListener('change',renderWorkOrders); $('#newWorkOrder').addEventListener('click',openWorkOrderDialog); $('#workOrderRobot').addEventListener('change',populateWorkOrderTechnicians); $('#workOrderCalendar').addEventListener('click',async (event) => { const button=event.target.closest('[data-work-order-id]'); if (!button) return; try { await api(`/api/v1/work-orders/${button.dataset.workOrderId}`,{ method:'PATCH',body:JSON.stringify({ status:button.dataset.workOrderStatus,completionNote:button.dataset.workOrderStatus === 'completed' ? 'Work completed and verified.' : '' }) }); toast('Work order updated'); await Promise.all([loadWorkOrders(),loadAudit()]); } catch(error){ toast(error.message,true); } });
  ...
  $('#workOrderForm').addEventListener('submit',async (event) => { if (event.submitter?.value === 'cancel') return; event.preventDefault(); const form=event.currentTarget; setFormBusy(form,true); try { await api('/api/v1/work-orders',{ method:'POST',body:JSON.stringify({ title:$('#workOrderName').value.trim(),robotId:$('#workOrderRobot').value,technicianId:$('#workOrderTechnician').value,startsAt:new Date($('#workOrderStarts').value).toISOString(),endsAt:new Date($('#workOrderEnds').value).toISOString(),priority:$('#workOrderPriority').value,description:$('#workOrderDescription').value.trim() }) }); $('#workOrderDialog').close(); toast('Work order scheduled'); await Promise.all([loadWorkOrders(),loadAudit(),loadNotifications()]); } catch(error){ toast(error.message,true); } finally { setFormBusy(form,false); } });
