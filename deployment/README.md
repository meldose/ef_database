...
6. Publish immutable backend and frontend images and set both `ALTEGRO_IMAGE` and `ALTEGRO_FRONTEND_IMAGE`. The workflow publishes both images with the Git commit SHA.
...
The GitHub deployment uses SSH port `222` by default; override it with the `DEPLOY_PORT` repository variable when needed. The deployment script performs a fail-fast preflight for immutable image names, the domain, legal identity, alert settings, Docker secrets and the merged Compose configuration. After rollout it verifies readiness, health, the frontend and both legal pages over normal HTTPS certificate validation. It will not start a partial production stack. Use secure files as input to `docker secret create`; never pass secret values directly on a command line or commit them to this repository.
